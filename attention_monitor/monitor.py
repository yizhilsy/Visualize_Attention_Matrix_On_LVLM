...
class AttentionMonitor:
    def __init__(self, model, model_layers, layer_id_list, tokenizer, image_processor):
        ...
        # 用于计数保存的注意力热力图样本
        self.save_attnheatmap_cnt = 0

        # 每个batch中的每个sample的统计结果
        self.batch_statistics = {layer_id: {
            'vision_to_vision': [],
            'vision_to_text': [],
            'text_to_vision': [],
            'text_to_text': []
        } for layer_id in layer_id_list}
        # 总体统计结果，所有batch中的sample结果
        self.overall_statistics = {layer_id: {
            'vision_to_vision': [],
            'vision_to_text': [],
            'text_to_vision': [],
            'text_to_text': []
    ...
    def _create_result_folder(self):
        ...
        # 生成随机uid作为文件夹名称
        uid = str(uuid.uuid4())
        self.result_folder = os.path.join(results_dir, uid)
    ...
    def _reset_batch_statistics(self):
        """
        重置当前batch的统计结果, 并且释放 self.attention_matrices 和 image_position_masks 中当前batch的数据
        """
        for layer_id in self.layer_id_list:
            self.batch_statistics[layer_id] = {
                'vision_to_vision': [],
            ...
            self.attention_matrices[layer_id] = []
            self.image_position_masks = []

    def _reset_batch_image_tensor(self):
        """
        重置 self.image_tensor_list 中当前 batch 的图片张量，为下一个 batch 做准备
        """
        self.image_tensor_list = []

    # 处理单个batch的统计结果，并存储到 batch_statistics 中
    def process_batch_statistics(self):
        """
        NOTE
        处理单个batch的统计，将当前batch的统计结果存储到batch_statistics成员变量中
        这个函数应该在每个batch推理结束后调用
        """
        # 1. 数据验证
    ...
    def _accumulate_batch_to_overall(self):
        ...
        for layer_id in self.layer_id_list:
            ...
            for mode in ['vision_to_vision', 'vision_to_text', 'text_to_vision', 'text_to_text']:
                self.overall_statistics[layer_id][mode].extend(self.batch_statistics[layer_id][mode])

    def overall_attention_statistic_summary(self) -> dict:
        """
        NOTE
        计算所有mode的总体汇总统计

        Returns:
            dict: 包含每个layer的所有mode统计和跨layer总体统计
        """
        # 初始化汇总统计信息
        ...
        # 按layer统计
        for layer_id, layer_stats in self.overall_statistics.items():
            for mode, scores in layer_stats.items():
                mode_total = sum(scores)
                # 累加到该层的对应模式总分
                layer_summary_stats[layer_id][mode] = mode_total
                layer_summary_stats[layer_id]['text_modality'] = layer_summary_stats[layer_id]['vision_to_text'] + layer_summary_stats[layer_id]['text_to_text']
                layer_summary_stats[layer_id]['vision_modality'] = layer_summary_stats[layer_id]['vision_to_vision'] + layer_summary_stats[layer_id]['text_to_vision']

        # 计算总体统计（跨所有层）
        overall_totals = {
            'vision_to_vision': sum(layer_summary_stats[layer_id]['vision_to_vision'] for layer_id in self.layer_id_list),
            'vision_to_text': sum(layer_summary_stats[layer_id]['vision_to_text'] for layer_id in self.layer_id_list),
            'text_to_vision': sum(layer_summary_stats[layer_id]['text_to_vision'] for layer_id in self.layer_id_list),
            'text_to_text': sum(layer_summary_stats[layer_id]['text_to_text'] for layer_id in self.layer_id_list),
            'text_modality': sum(layer_summary_stats[layer_id]['text_modality'] for layer_id in self.layer_id_list),
            'vision_modality': sum(layer_summary_stats[layer_id]['vision_modality'] for layer_id in self.layer_id_list)
        }

        # 保存统计结果到JSON文件
        statistics_data = {
            'raw_statistics': self.overall_statistics,
            'layer_summary': layer_summary_stats,
            'overall_totals': overall_totals,
            'metadata': {
                'total_samples': sum(len(scores) for layer_stats in self.overall_statistics.values() for scores in layer_stats.values()) // 4,
                'num_layers': len(self.layer_id_list),
                'layer_ids': list(self.layer_id_list)
            }
        }

        statistics_file = os.path.join(self.result_folder, "overall_attention_statistics.json")
        with open(statistics_file, 'w', encoding='utf-8') as f:
            json.dump(statistics_data, f, ensure_ascii=False, indent=2)

        print(f"Overall attention statistics saved to: {statistics_file}")
    ...
    def plot_batch_attention_heatmap(self):
        """
        NOTE
        处理当前batch的注意力热力图可视化，为每个sample生成独立文件夹保存热力图
        使用save_attnheatmap_cnt来计数和命名样本文件夹
        直接遍历self.attention_matrices和self.image_position_masks
        """
        print("\n=== GENERATING CURRENT BATCH ATTENTION HEATMAP VISUALIZATIONS ===")
        ...
        if not self.attention_matrices or not self.image_position_masks:
            print("Warning: No attention matrices or image position masks found. Skipping heatmap generation.")
            return

        # 创建热力图存储文件夹
        heatmap_dir = os.path.join(self.result_folder, "attn_heatmap")
        ...
        BATCH_SIZE = None
        # 遍历所有batch
        for batch_idx in range(len(self.image_position_masks)):
            # 遍历所有层
            for layer_id in self.layer_id_list:
                # 检查该层是否有对应batch的注意力矩阵
                if batch_idx >= len(self.attention_matrices[layer_id]):
                    continue

                layer_attention_matrices = self.attention_matrices[layer_id]
                attention_matrix = layer_attention_matrices[batch_idx]
                # attention_matrix shape: (bs, attn_head, seq_len, seq_len)
                bs, num_heads, seq_len, _ = attention_matrix.shape
                ...
                for sample_idx in range(bs):
                    ...
                    # 获取对应的原始图像
                    image_list_idx = batch_idx * bs + sample_idx  # 计算当前样本在整个推理过程中的索引
                    if image_list_idx >= len(self.image_tensor_list):
                        print(f"Warning: No image found for sample {self.save_attnheatmap_cnt + batch_idx * bs + sample_idx}.")
                        continue

                    original_image = self.image_tensor_list[image_list_idx]  # 形状: (3, h, w)
                    if isinstance(original_image, torch.Tensor):
                        original_image = original_image.to(torch.float32).cpu().numpy()
                    ...
                    # 创建样本专属的文件夹
                    sample_dir = os.path.join(heatmap_dir, f"sample_{self.save_attnheatmap_cnt + batch_idx * bs + sample_idx}")
                    ...
                    if not grid_sizes:
                        ...
                        print(f"Sample {self.save_attnheatmap_cnt}: Found grid size {grid_size} for {num_vision_tokens} tokens")

                    # 创建2D注意力图
                    attention_map = np.zeros(grid_size)

                    # 填充注意力分数
                    for idx, score in enumerate(avg_vision_scores.tolist()):
                    ...
                    plt.suptitle(f'Layer {layer_id} - Batch {batch_idx} - Sample {sample_idx}', fontsize=16)
                    plt.tight_layout()
                    ...
                    plt.imshow(overlay)
                    plt.title(f'Layer {layer_id} - Batch {batch_idx} - Sample {sample_idx}', fontsize=16)
                    plt.axis('off')
                    plt.savefig(os.path.join(sample_dir, f"layer_{layer_id}_overlay.png"),
                               dpi=300, bbox_inches='tight', pad_inches=0.1)
                    plt.close()

                    print(f"  Sample {self.save_attnheatmap_cnt + batch_idx * bs + sample_idx} layer {layer_id} heatmap saved")

        self.save_attnheatmap_cnt += len(self.image_position_masks) * BATCH_SIZE
        print(f"\nAll batches heatmap visualizations completed.")
